When('I add the first product to the cart') do
  ...
  expect(add_button).not_to be_nil, "No 'Add to cart' button found."
  add_button.click
  expect(@checkout_page.cart_badge_count).to eq(1)
end

When('I add the second product to the cart') do
  add_button = first('.inventory_item .btn_primary.btn_inventory', wait: 5)
  expect(add_button).not_to be_nil, "No second product 'Add to cart' button found."
  add_button.click
  expect(@checkout_page.cart_badge_count).to eq(2)
end
...
When('I fill in my information with first name {string}, last name {string}, and postal code {string}') do |first, last, zip|
  @checkout_page.fill_information(first_name: first, last_name: last, postal_code: zip)
end
...
Then('I should see the confirmation message {string}') do |msg|
  expect(@checkout_page.confirmation_message).to eq(msg)
end
...
Then('the total should be correct') do
  expect(@checkout_page.total_correct?).to be true
end
...
Then('I should be on the checkout overview page') do
  expect(@checkout_page.on_overview_page?).to be true
...
Then('I should be on the checkout information page') do
  expect(@checkout_page.on_information_page?).to be true
